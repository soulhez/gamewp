...
?>
			...
			<p>
				<label for="recent-comments-number"><?php _e('Number of comments to show:','addict'); ?> <input style="width: 25px; text-align: center;" id="recent-comments-number" name="wpwall-recent-comments-number" type="text" value="<?php echo $number; ?>" />
				</label>
				<br />
				<small><?php _e('(at most 15)', 'addict'); ?></small>
			</p>
...
<?php
  ...
  function display($args) {
	...
	$options = get_option('wpwall_widget_recent_comments');
	$title = empty($options['title']) ? __('Recent Comments', 'addict') : apply_filters('widget_title', $options['title']);
...
?>
		...
		<?php echo $before_widget; ?>
			...
			echo  '<li class="recentcomments">' . sprintf(__('%1$s on %2$s', 'addict'), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a>') . '</li>';
			endforeach; endif;?></ul>
